...
public class Configuration {
	...
	public boolean analyze() {
		...
		if (status) {
			...
			if (linkingUsesKey) {
				...
				status = status & temp;
				if (temp) {
					checkKeyColumnForKey("player-user-linking.key-name", linkingTableName, linkingKeyColumn,
							linkingKeyName);
				}

				status = status & checkColumn("player-user-linking.value-column", linkingTableName, linkingValueColumn);
			} else {
				status = status & checkColumn("player-user-linking.identifier-column", linkingTableName,
						linkingIdentifierColumn);
			}
		}
		...
		if (postCountEnabled) {
			...
			status = status & temp;
			if (temp) {
				temp = temp & checkColumn("app-post-count-config.user-id-column", postCountTableName,
						postCountUserIDColumn);
				temp = temp & checkColumn("app-post-count-config.post-count-column", postCountTableName,
						postCountPostCountColumn);
			}
			if (!temp) {
		...
		if (walletEnabled) {
			...
			if (temp) {
				...
				if (walletUsesKey) {
					...
					checkKeyColumnForKey("app-wallet-config.column-or-key-name", walletTableName, walletKeyColumn,
							walletColumnOrKey);
				} else {
					temp = temp
							& checkColumn("app-wallet-config.column-or-key-name", walletTableName, walletColumnOrKey);
				}
			}
			if (!temp) {
		...
		if (statisticsEnabled) {
			...
			if (temp) {
				...
				if (statisticsUsesInsert && statisticsInsertMethod.startsWith("smf")) {
					status = status
							& checkColumn("statistics.theme-id-column", statisticsTableName, statisticsThemeIDColumn);
					checkKeyColumnForKey("statistics.theme-id", statisticsTableName, statisticsThemeIDColumn,
							statisticsThemeID);
				}
				...
				if (statisticsUsesKey) {
					...
					if (temp) {
						if (onlineStatusEnabled) {
							checkKeyColumnForKey("statistics.trackers.online-status.column-or-key-name",
									statisticsTableName, statisticsKeyColumn, onlineStatusColumnOrKey);
						}
						if (lastonlineEnabled) {
							checkKeyColumnForKey("statistics.trackers.last-online.column-or-key-name",
									statisticsTableName, statisticsKeyColumn, lastonlineColumnOrKey);
							if (!lastonlineFormattedColumnOrKey.isEmpty()) {
								checkKeyColumnForKey("statistics.trackers.last-online.formatted-column-or-key-name",
										statisticsTableName, statisticsKeyColumn, lastonlineFormattedColumnOrKey);
							}
						}
						if (gametimeEnabled) {
							checkKeyColumnForKey("statistics.trackers.game-time.column-or-key-name",
									statisticsTableName, statisticsKeyColumn, gametimeColumnOrKey);
							if (!gametimeFormattedColumnOrKey.isEmpty()) {
								checkKeyColumnForKey("statistics.trackers.game-time.formatted-column-or-key-name",
										statisticsTableName, statisticsKeyColumn, gametimeFormattedColumnOrKey);
							}
							if (!lastonlineEnabled) {
								log.warning(
										"Game time statistic tracker requires last online tracker to be enabled. Temporarily disabling gametime tracker.");
								gametimeEnabled = false;
							}
						}
						if (levelEnabled) {
							checkKeyColumnForKey("statistics.trackers.level.column-or-key-name", statisticsTableName,
									statisticsKeyColumn, levelColumnOrKey);
						}
						if (totalxpEnabled) {
							checkKeyColumnForKey("statistics.trackers.total-xp.column-or-key-name", statisticsTableName,
									statisticsKeyColumn, totalxpColumnOrKey);
						}
						if (currentxpEnabled) {
							checkKeyColumnForKey("statistics.trackers.current-xp.column-or-key-name",
									statisticsTableName, statisticsKeyColumn, currentxpColumnOrKey);
							if (!currentxpFormattedColumnOrKey.isEmpty()) {
								checkKeyColumnForKey("statistics.trackers.current-xp.formatted-column-or-key-name",
										statisticsTableName, statisticsKeyColumn, currentxpFormattedColumnOrKey);
							}
						}
						if (healthEnabled) {
							checkKeyColumnForKey("statistics.trackers.health.column-or-key-name", statisticsTableName,
									statisticsKeyColumn, healthColumnOrKey);
						}
						if (lifeticksEnabled) {
							checkKeyColumnForKey("statistics.trackers.lifeticks.column-or-key-name",
									statisticsTableName, statisticsKeyColumn, lifeticksColumnOrKey);
							if (!lifeticksFormattedColumnOrKey.isEmpty()) {
								checkKeyColumnForKey("statistics.trackers.lifeticks.formatted-column-or-key-name",
										statisticsTableName, statisticsKeyColumn, lifeticksFormattedColumnOrKey);
							}
						}
					}
				} else {
					if (onlineStatusEnabled && !checkColumn("statistics.trackers.online-status.column-or-key-name",
							statisticsTableName, onlineStatusColumnOrKey)) {
						onlineStatusEnabled = false;
					}

					if (lastonlineEnabled) {
						if (!checkColumn("statistics.trackers.last-online.column-or-key-name", statisticsTableName,
								lastonlineColumnOrKey)) {
							lastonlineEnabled = false;
						}
						if (!lastonlineFormattedColumnOrKey.isEmpty()
								&& !checkColumn("statistics.trackers.last-online.formatted-column-or-key-name",
										statisticsTableName, lastonlineFormattedColumnOrKey)) {
							lastonlineFormattedColumnOrKey = "";
						}
					}

					if (gametimeEnabled) {
						if (!checkColumn("statistics.trackers.game-time.column-or-key-name", statisticsTableName,
								gametimeColumnOrKey)) {
							gametimeEnabled = false;
						}

						if (!gametimeFormattedColumnOrKey.isEmpty()
								&& !checkColumn("statistics.trackers.game-time.formatted-column-or-key-name",
										statisticsTableName, gametimeFormattedColumnOrKey)) {
							gametimeFormattedColumnOrKey = "";
						}

						if (!lastonlineEnabled) {
							log.warning(
									"Gametime tracker requires lastonline tracker to be enabled. Temporarily disabling gametime tracker.");
							gametimeEnabled = false;
							gametimeFormattedColumnOrKey = "";
						}
					}

					if (levelEnabled && !checkColumn("statistics.trackers.level.column-or-key-name",
							statisticsTableName, levelColumnOrKey)) {
						levelEnabled = false;
					}

					if (totalxpEnabled && !checkColumn("statistics.trackers.total-xp.column-or-key-name",
							statisticsTableName, totalxpColumnOrKey)) {
						totalxpEnabled = false;
					}

					if (currentxpEnabled) {
						if (!checkColumn("statistics.trackers.current-xp.column-or-key-name", statisticsTableName,
								currentxpColumnOrKey)) {
							currentxpEnabled = false;
						}

						if (!currentxpFormattedColumnOrKey.isEmpty()
								&& !checkColumn("statistics.trackers.current-xp.formatted-column-or-key-name",
										statisticsTableName, currentxpFormattedColumnOrKey)) {
							currentxpFormattedColumnOrKey = "";
						}
					}

					if (healthEnabled && !checkColumn("statistics.trackers.health.column-or-key-name",
							statisticsTableName, healthColumnOrKey)) {
						healthEnabled = false;
					}

					if (lifeticksEnabled) {
						if (!checkColumn("statistics.trackers.lifeticks.column-or-key-name", statisticsTableName,
								lifeticksColumnOrKey)) {
							lifeticksEnabled = false;
						}

						if (!lifeticksFormattedColumnOrKey.isEmpty()
								&& !checkColumn("statistics.trackers.lifeticks.formatted-column-or-key-name",
										statisticsTableName, lifeticksFormattedColumnOrKey)) {
							lifeticksFormattedColumnOrKey = "";
						}
					}

					if (!(onlineStatusEnabled || lastonlineEnabled || gametimeEnabled || levelEnabled || totalxpEnabled
							|| currentxpEnabled || healthEnabled || lifeticksEnabled)) {
						log.warning(
								"Statistics tracking is enabled, but none of the individual trackers are enabled. Temporarily disabling statistics tracking.");
		...
		if (webappPrimaryGroupEnabled) {
			...
			temp = temp & checkColumn("app-group-config.primary.user-id-column", webappPrimaryGroupTable,
					webappPrimaryGroupUserIDColumn);
			temp = temp & checkColumn("app-group-config.primary.group-id-column", webappPrimaryGroupTable,
					webappPrimaryGroupGroupIDColumn);
			if (webappPrimaryGroupUsesKey) {
				temp = temp & checkColumn("app-group-config.primary.key-column", webappPrimaryGroupTable,
						webappPrimaryGroupKeyColumn);
				if (temp) {
					checkKeyColumnForKey("app-group-config.primary.key-name", webappPrimaryGroupTable,
							webappPrimaryGroupKeyColumn, webappPrimaryGroupKeyName);
				} else {
		...
		if (webappSecondaryGroupEnabled) {
			...
			temp = temp & checkColumn("app-group-config.secondary.user-id-column", webappSecondaryGroupTable,
					webappSecondaryGroupUserIDColumn);
			temp = temp & checkColumn("app-group-config.secondary.group-id-column", webappSecondaryGroupTable,
					webappSecondaryGroupGroupIDColumn);
			if (webappSecondaryGroupStorageMethod.startsWith("mul")
					|| webappSecondaryGroupStorageMethod.startsWith("key")) {
				temp = temp & checkColumn("app-group-config.secondary.key-column", webappSecondaryGroupTable,
						webappSecondaryGroupKeyColumn);
				if (temp) {
					checkKeyColumnForKey("app-group-config.secondary.key-name", webappSecondaryGroupTable,
							webappSecondaryGroupKeyColumn, webappSecondaryGroupKeyName);
				}
			}
			if (webappSecondaryGroupStorageMethod.startsWith("jun")) {
				for (String columnName : webappSecondaryAdditionalColumns.keySet()) {
					temp = temp & checkColumn("app-group-config.secondary.additional-columns." + columnName,
							webappSecondaryGroupTable, columnName);
				}
			}
			if (!temp) {
		...
		if (simpleSynchronizationEnabled && webappPrimaryGroupEnabled == false
				&& webappSecondaryGroupEnabled == false) {
			simpleSynchronizationEnabled = false;
			log.severe("Simple synchronization disabled due to prior errors.");
		}

		// This one needs to be performed after the one above, in case the one above
		// disables sync.
		...
		if (simpleSynchronizationEnabled && webappPrimaryGroupEnabled
				&& environment.getPermissionHandler().supportsPrimaryGroups() == false
				&& simpleSynchronizationGroupsTreatedAsPrimary.isEmpty()) {
			log.warning(
					"The permissions system does not support primary groups and primary group synchronization is enabled...but there are no groups listed in the 'groups treated as primary' configuration option.");
		}

		if (banSynchronizationEnabled) {
			temp = checkTable("ban-synchronization.table-name", banSynchronizationTableName);
			temp = temp & checkColumn("ban-synchronization.banned-user-id-column", banSynchronizationTableName,
					banSynchronizationUserIDColumn);
			if (banSynchronizationMethod.startsWith("tab")) {
				temp = temp & checkColumnIfNotEmpty("ban-synchronization.ban-reason-column",
						banSynchronizationTableName, banSynchronizationReasonColumn);
				temp = temp & checkColumnIfNotEmpty("ban-synchronization.ban-start-column", banSynchronizationTableName,
						banSynchronizationStartTimeColumn);
				temp = temp & checkColumnIfNotEmpty("ban-synchronization.ban-end-column", banSynchronizationTableName,
						banSynchronizationEndTimeColumn);
				temp = temp & checkColumnIfNotEmpty("ban-synchronization.ban-group-id-column",
						banSynchronizationTableName, banSynchronizationBanGroupIDColumn);
			} else if (banSynchronizationMethod.startsWith("use")) {
				temp = temp & checkColumn("ban-synchronization.ban-column", banSynchronizationTableName,
						banSynchronizationBanColumn);
		...
		if (playerDataRequired) {
			...
			if (playerData.exists()) {
				if (!playerData.isDirectory()) {
					log.severe(
							"There is a file named Players in the CommunityBridge plugin folder preventing creation of the data directory.");
	...
	private boolean checkColumn(String keyName, String tableName, String columnName) {
		ResultSet result;
		String errorBase;
		errorBase = "Error while checking '" + keyName + "' set to '" + columnName + "': ";
	...
	private void checkKeyColumnForKey(String yamlKeyName, String tableName, String keyColumn, String keyName) {
		...
		String query = "SELECT COUNT(*) FROM `" + tableName + "` " + "WHERE `" + keyColumn + "` = '" + keyName + "'";
		...
		try {
			...
			if (result.next()) {
				if (result.getInt(1) == 0) {
					log.warning("There are no rows containing " + keyName + " in the " + keyColumn + " column, on the "
							+ tableName + " table.");
				}
			} else {
	...
	private boolean checkTable(String keyName, String tableName) {
		ResultSet result;
		String errorBase;
		errorBase = "Error while checking '" + keyName + "' set to '" + tableName + "': ";
	...
	private void loadSettings(FileConfiguration config) {
		...
		linkingRegisteredFormerUnregisteredOnly = config
				.getBoolean("player-user-linking.registered-former-unregistered-only", true);
		linkingUnregisterFormerRegistered = config.getBoolean("player-user-linking.unregister-former-registered",
				false);
		...
		if (linkingIdentifierColumn.isEmpty()) {
			...
			if (!linkingIdentifierColumn.isEmpty()) {
				log.severe(
						"the configuration option player-user-linking.playername-column is deprecated. Change playername-column to identifier-column in the player-user-linking section.");
			}
		}
		...
		lastonlineFormattedColumnOrKey = config
				.getString("statistics.trackers.last-online.formatted-column-or-key-name", "");
		...
		if (oldStatistic && !oldColumn.isEmpty()) {
			environment.getLog().warning(
					"Wallet configuration in the statistics section is deprecated. Use the new app-wallet-config section instead.");
			walletEnabled = true;
		...
		if (config.contains("app-group-config.secondary.additional-columns")) {
			webappSecondaryAdditionalColumns = config
					.getConfigurationSection("app-group-config.secondary.additional-columns").getValues(false);
		}
		...
		simpleSynchronizationFirstDirection = config.getString("simple-synchronization.first-direction", "two-way")
				.toLowerCase();
		simpleSynchronizationPrimaryGroupNotify = config
				.getBoolean("simple-synchronization.primary-group-change-notify", false);

		if (config.contains("simple-synchronization.group-mapping")) {
			simpleSynchronizationGroupMap = config.getConfigurationSection("simple-synchronization.group-mapping")
					.getValues(false);
		}

		if (config.contains("simple-synchronization.groups-treated-as-primary")) {
			simpleSynchronizationGroupsTreatedAsPrimary = config
					.getStringList("simple-synchronization.groups-treated-as-primary");
		}

		if (config.contains("simple-synchronization.webapp-secondary-groups-treated-as-primary")) {
			simpleSynchronizationWebappSecondaryGroupsTreatedAsPrimary = config
					.getStringList("simple-synchronization.webapp-secondary-groups-treated-as-primary");
		}
		...
		groupSynchronizationActive = simpleSynchronizationEnabled
				&& (webappPrimaryGroupEnabled || webappSecondaryGroupEnabled);
		playerDataRequired = groupSynchronizationActive || walletEnabled;
		playerSynchronizerRequired = groupSynchronizationActive || statisticsEnabled || useAchievements
				|| walletEnabled;
		permissionsSystemRequired = !linkingUnregisteredGroup.isEmpty() || !linkingRegisteredGroup.isEmpty()
				|| groupSynchronizationActive;
	...
	private PermissionHandler selectPermissionsHandler() {
		try {
			if (permissionsSystem.equalsIgnoreCase("PEX")) {
			...
			} else if (permissionsSystem.equalsIgnoreCase("LuckPerms")) {
				environment.getLog().config("Permissions System: LuckPerms");
				...
				environment.getLog().severe(
						"Unknown permissions system in config.yml. Features dependent on a permissions system disabled.");
	...
	private void loadMessages() {
		...
		if (rootSet.isEmpty()) {
			...
			log.warning(
					"Multiple top level keys in messages.yml. Assuming the first top level key is the correct one.");
		}
	...
	public final void report() {
		...
		if (webappSecondaryGroupEnabled) {
			...
			if (webappSecondaryGroupStorageMethod.startsWith("sin")
					|| webappSecondaryGroupStorageMethod.startsWith("key")) {
		...
		if (simpleSynchronizationEnabled) {
			...
			log.config(
					"Simple synchronization P-groups      : " + simpleSynchronizationGroupsTreatedAsPrimary.toString());
			log.config("Simple synchronization WSP-groups    : "
					+ simpleSynchronizationWebappSecondaryGroupsTreatedAsPrimary.toString());
		}
	...
	private boolean checkSuperUserID() {
		...
		String query = "SELECT `" + linkingUserIDColumn + "`" + " FROM `" + linkingTableName + "`" + " WHERE `"
				+ linkingUserIDColumn + "` = '" + simpleSynchronizationSuperUserID + "'";
	...
	private boolean enableSQL(boolean reload) {
		...
		environment.setSql(new SQL(environment.getLog(), databaseHost + ":" + databasePort, databaseName + "",
				databaseUsername + "", databasePassword + "", databaseBindingAddress));
